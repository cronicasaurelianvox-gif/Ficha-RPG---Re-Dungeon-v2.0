...
class VeiasAstraisSystem {
    ...
    showNotification(message, type = 'info', duration = 3000) {
        ...
        const colors = {
            success: { bg: 'rgba(102, 221, 102, 0.15)', text: '#66dd66', border: '#66dd66' },
            error: { bg: 'rgba(255, 68, 68, 0.15)', text: '#ff6b6b', border: '#ff4444' },
            warning: { bg: 'rgba(255, 221, 68, 0.15)', text: '#ffdd44', border: '#ffbb00' },
            info: { bg: 'rgba(68, 119, 221, 0.15)', text: '#4477dd', border: '#4477dd' }
        };
    ...
    syncPowerCombatFromAppState() {
        if (!window.powerCombatCalculator) {
            console.warn('⚠️ PowerCombatCalculator não disponível para sincronização');
            return;
        }
        ...
        try {
            ...
            if (maxMudou) {
                ...
                console.log(`📈 Máximo Power Combat atualizado: ${this.maxPowerCombat} → ${novoMaxPowerCombat}`);
                ...
                if (novoMaxPowerCombat > antigoMaxPowerCombat) {
                    ...
                    console.log(`⬆️ Máximo aumentou! Adicionando incremento de ${incremento} ao PC atual`);
                    console.log(`   PC anterior: ${this.powerCombat} → PC novo: ${novoPC}`);
                ...
                // Se o máximo diminuiu e PC ficaria acima do limite, reduzir
                    ...
                    console.log(`📉 PC atual reduzido de ${this.powerCombat} para ${novoMaxPowerCombat} (excedia o máximo)`);
                ...
                if (this.powerCombat === 0 && this.maxPowerCombat > 0) {
                    ...
                    console.log(`✨ Primeira sincronização: Power Combat definido como ${this.powerCombat}`);
                }
    ...
    setupPowerCombatListener() {
        ...
        if (window.powerCombatCalculator) {
            window.powerCombatCalculator.onChange((novoValor) => {
                console.log(`🔥 Power Combat mudou para ${novoValor}! Sincronizando imediatamente...`);
                this.syncPowerCombatFromAppState();
            });
            console.log('✅ Callback de Power Combat registrado para atualização imediata');
        }
        ...
        console.log('👁️ Listener de Power Combat ativado (imediato + polling 500ms)');
    }
    ...
    generateTrees() {
        this.trees = {
            arty: {
                ...
                icon: '⚔️',
                symbol: '⚡',
                description: 'A Entidade do Caos e Destruição - Poder bruto, violência e colapso cósmico',
                loreDescription: 'Arty sussurra através das Veias Astrais do poder destrutivo. Seus seguidores canalizam agressividade pura e energia descontrolada.',
            ...
            aune: {
                ...
                icon: '✨',
                symbol: '★',
                description: 'A Entidade do Destino e Fortuna - Coincidência cósmica e inevitabilidade',
                loreDescription: 'Aune tece os fios da inevitabilidade. Quem recebe sua bênção comanda a probabilidade e o destino cósmico.',
            ...
            ephelias: {
                ...
                icon: '📚',
                symbol: '◆',
                description: 'A Entidade da Sabedoria - Inteligência infinita e conhecimento arcano proibido',
                loreDescription: 'Ephelias revela segredos antigos. Seus aprendizes ganham acesso ao conhecimento infinito e manipulação arcana.',
            ...
            nishi: {
                ...
                icon: '🌀',
                symbol: '◇',
                description: 'A Entidade do Equilíbrio - Precisão, harmonia e fluxo perfeito',
                loreDescription: 'Nishi mantém a harmonia entre forças opostas. Seus seguidores alcançam velocidade, precisão e fluxo perfeito do cosmos.',
            ...
            hestia: {
                ...
                icon: '🌿',
                symbol: '❖',
                description: 'A Entidade da Criação - Vitalidade, regeneração e sustentação cósmica',
                loreDescription: 'Hestia nutre toda a existência. Seus eleitos canalizam vida, proteção divina e crescimento sem limites.',
        ...
        console.log('✨ Constelações Divinas geradas:', Object.keys(this.trees).length);
    }
    ...
    generateNodes() {
        ...
        const treeConfigs = [
            ...
            { id: 'ephelias', angle: 144, color: '#4477dd', name: 'Ephelias', rarity: 3 },
        ...
        treeConfigs.forEach(treeConfig => {
            ...
            const layer2Nodes = this.nodes.filter(n => n.treeId === treeConfig.id && n.layer === 2);
            for (let i = 0; i < 3; i++) {
            ...
            const layer3Nodes = this.nodes.filter(n => n.treeId === treeConfig.id && n.layer === 3);
            for (let i = 0; i < 2; i++) {
            ...
            const layer4Nodes = this.nodes.filter(n => n.treeId === treeConfig.id && n.layer === 4);
        ...
        console.log(`✨ ${this.nodes.length} nós orgânicos gerados em hierarquia 5 camadas`);
    }
    ...
    generateConnections() {
        ...
        console.log(`✨ ${this.connections.length} veias energéticas orgânicas geradas`);
    }
    ...
    renderNodes() {
        ...
        this.nodes.forEach(node => {
            ...
            if (node.state === 'locked') {
                ...
                content = '<span class="node-crown">👑</span><span class="node-rune">' + node.rune + '</span>';
            } else {
    ...
    renderConnections() {
        ...
        this.connections.forEach((conn, idx) => {
            ...
            const bothUnlocked = startNode.state === 'unlocked' && endNode.state === 'unlocked';
        ...
        console.log(`✨ ${this.connections.length} veias energéticas renderizadas com padrões distintos`);
    }
    ...
    renderNodeDetails() {
        ...
        const blockReasonSection = document.getElementById('node-block-reason-section');
    ...
    unlockNode(node) {
        ...
        if (this.powerCombat < pcNecessario) {
            ...
            this.showNotification(`❌ PC insuficiente! Faltam ${pcInsuficiente} PC`, 'error', 3000);
            return;
        }
        ...
        // Log e notificação
        if (caminho.length > 1) {
            console.log(`🔓 Caminho desbloqueado até "${node.name}": ${caminho.length} nó(s)`);
            console.log(`   Nós desbloqueados: ${nomesNosDesbloqueados.join(' → ')}`);
            console.log(`   Total de PC gasto: ${pcGasto}`);
            this.showNotification(`🔓 Caminho aberto até ${node.name}! -${pcGasto} PC`, 'success', 4000);
        } else {
            console.log(`🔓 Nó "${node.name}" desbloqueado. PC gasto: ${pcGasto}`);
            this.showNotification(`✅ ${node.name} desbloqueado. -${pcGasto} PC`, 'success', 3000);
        }
    ...
    findDescendantNodes(node, visited = new Set()) {
        ...
        const childConnections = this.connections.filter(conn => conn.from === node.id);
    ...
    lockNode(node) {
        ...
        nodosABloquear.forEach(no => {
            if (no.state !== 'locked') {
                // Recuperar PC
                this.powerCombat = Math.min(this.maxPowerCombat, this.powerCombat + no.cost);
        ...
        if (descendentes.length > 0) {
            console.log(`🔒 Nó "${node.name}" bloqueado. ${descendentes.length} nó(s) posterior(es) também bloqueado(s).`);
            console.log(`   Nós bloqueados: ${nomesNosDesbloqueados.join(', ')}`);
            console.log(`   Total de PC recuperado: ${pcRecuperado}`);
            this.showNotification(`🔒 ${nomesNosDesbloqueados.length} nó(s) bloqueado(s). +${pcRecuperado} PC`, 'warning', 4000);
        } else {
            console.log(`🔒 Nó "${node.name}" bloqueado. PC recuperado: ${pcRecuperado}`);
            this.showNotification(`✅ ${node.name} bloqueado. +${pcRecuperado} PC`, 'success', 3000);
        }
    ...
    updateUI() {
        ...
        if (powerCombatFill) powerCombatFill.style.width = `${(this.powerCombat / this.maxPowerCombat) * 100}%`;
        ...
        if (resonanceFill) resonanceFill.style.width = `${(this.resonance / this.maxResonance) * 100}%`;
    }
    ...
    generateNodeName(treeId, index) {
        const names = {
            arty: ['Instinto Primitivo', 'Primeira Destruição', 'Aura Caótica', 'Cólera Divina', 'Fúria Cósmica', 'Ruptura da Realidade', 'Poder do Caos', 'Arty Supremo'],
            aune: ['Presságio Suave', 'Linha do Destino', 'Tecelã Celestial', 'Sincronicidade', 'Trilho Inevitável', 'Fortuna Cósmica', 'Vontade do Universo', 'Aune Absoluta'],
            ephelias: ['Revelação Primeira', 'Símbolo Arcano', 'Grimório Antigo', 'Conhecimento Proibido', 'Verdade Oculta', 'Sabedoria Infinita', 'Consciência Cósmica', 'Ephelias Suprema'],
            nishi: ['Respiração Cósmica', 'Fluxo Perfeito', 'Movimento Celeste', 'Sincronismo', 'Ritmo do Universo', 'Harmonia Plena', 'Dança das Esferas', 'Nishi Absoluta'],
            hestia: ['Semente da Vida', 'Bênção Vital', 'Energia Criadora', 'Florescimento', 'Raiz Profunda', 'Força Regenerativa', 'Abraço do Universo', 'Hestia Suprema']
        };
    ...
    generateNodeDescription(treeId, index) {
        ...
        return descriptions[treeId] || 'Uma bênção divina flui através das Veias Astrais.';
    }
